...
/*   Updated: 2020/03/11 03:21:07 by loamar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void    get_distances_no_so(t_player *player, int column, int step_x, int side)
{
    player->distances[column] = fabs((player->map_x - player->ray_pos_x + (1 - step_x) / 2) / player->raydir_x);
    player->hit_wall_x[column] = (float)player->ray_pos_y + (((float)player->map_x - player->ray_pos_x + (1. - (float)step_x) / 2.) / player->raydir_x) * player->raydir_y;
...
static void    get_distances_we_ea(t_player *player, int column, int step_y, int side)
{
    player->distances[column] = fabs((player->map_y - player->ray_pos_y + (1 - step_y) / 2) / player->raydir_y);
    player->sides[column] = side;
    player->hit_wall_x[column] = (float)player->ray_pos_x + (((float)player->map_y - player->ray_pos_y + (1. - (float)step_y) / 2.) / player->raydir_y) * player->raydir_x;
    player->hit_wall_x[column] -= (int)player->hit_wall_x[column];
    player->tex_x[column] = (int)(player->hit_wall_x[column] * 64);
...
static void    set_dda_value(t_player *player, int *step_x, int *step_y)
{
    ...
    {
        *step_x = -1;
        player->sidedist_x = (player->ray_pos_x - player->map_x) * player->deltadist_x;
    }
    else
    {
        *step_x = 1;
        player->sidedist_x = (player->map_x + 1.0 - player->ray_pos_x) * player->deltadist_x;
    }
    if (player->raydir_y < 0)
    {
        *step_y = -1;
        player->sidedist_y = (player->ray_pos_y - player->map_y) * player->deltadist_y;
    }
    else
    {
        *step_y = 1;
        player->sidedist_y = (player->map_y + 1.0 - player->ray_pos_y) * player->deltadist_y;
    }
}
...
int     ray_casting(t_cub3d *cub)
{
    ...
    column = 0;
    // cub->player->dir_x = 0.5;
    while (column < cub->map->res_x)
    {
        // if (moove_fly(cub, column) == 0)
        // {
        ...
        //     // * cub->player->raydir_y) / (cub->player->raydir_x * cub->player->raydir_x)) / 2;
        //     // cub->player->deltadist_y = sqrt(1 + (cub->player->raydir_x
        //     // * cub->player->raydir_x) / (cub->player->raydir_y * cub->player->raydir_y));
        // }
        // else
        // {
            cub->player->deltadist_x =  sqrt(1 + (cub->player->raydir_y
            * cub->player->raydir_y) / (cub->player->raydir_x * cub->player->raydir_x));
            cub->player->deltadist_y = sqrt(1 + (cub->player->raydir_x
            * cub->player->raydir_x) / (cub->player->raydir_y * cub->player->raydir_y));
        // }
        algo_dda(cub, column++);
